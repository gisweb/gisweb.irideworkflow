...
def get_cointestatario(rec):
    ...
    gridfields = fld.field_mapping.split(',')
    nfo = dict(zip(gridfields, rec))

    cointestatario = dict(
        CodiceFiscale = lambda doc: doc['fisica_cf_cointestatari'],
        CognomeNome = lambda doc: '%s %s' % (doc['fisica_cognome_cointestatari'], doc['fisica_nome_cointestatari'], ),
        Indirizzo = lambda doc: '%s, %s' % (doc['fisica_indirizzo_cointestatari'], doc['fisica_civico_cointestatari'], ),
        CodiceComuneResidenza = lambda doc: doc['fisica_cod_cat_cointestatari'],
        DataNascita = lambda doc: doc['fisica_data_nato_cointestatari'],
        CodiceComuneNascita = lambda doc: doc['fisica_cod_cat_nato_cointestatari'],
        Nazionalita = lambda doc: 'IT' if doc['fisica_cod_cat_nato_cointestatari'] != 'EE' else doc['fisica_comune_nato_cointestatari'],
        TipoSogg = lambda doc: 'S',
        TipoPersona = lambda doc: 'FI',
    )

    return dict([(k,f(rec)) for k,f in cointestatario.items()])
